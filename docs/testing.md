...
The workflow requires exactly one setup executable, uploads it with `if-no-files-found: error`, and uploads failure diagnostics with `if-no-files-found: warn`. For v0.1.3 the expected installer is `dsh-flightdeck-0.1.3-dsh-0.1.0-rc.7-windows-x64-setup.exe`. It creates no GitHub Release.

## Release contracts

`scripts/release-metadata.mjs` derives the Flightdeck version, bundled DSH version, exact Windows and macOS asset names, prerelease state, and `release_notes_path=docs/releases/${expectedTag}.md` from the package metadata. Release-note contract: if that file exists it is used as the declared description; if it is absent, `scripts/prepare-release-notes.mjs` generates exactly one line — `**Full Changelog**: https://github.com/Zeno2019/dsh-flightdeck/compare/<previous-tag>...<current-tag>` — and the published release shows only this compare. The workflow runs the prepare step before verifying the notes file, then builds. The Windows job creates or updates a Draft with `--notes-file` and uploads the NSIS asset. The macOS job builds the target-gated arm64 DMG, uploads it to the same Draft, verifies both exact assets, and only then publishes. Release packages remain unsigned; the unsigned-package warnings and limited-validation disclosures live in the README.
