...
- `scripts/prepare-profile-web.mjs` stages `build/profile-web/payload` at packaging time: the profile manifest (template bundles `@deepseek-ai/dsh-base` and `@deepseek-ai/dsh-web-app` plus the three approved plugins), an empty `cordis.patch.yml`, the DSH profile's own `pnpm-workspace.yaml` (byte-identical to dsh-app-boot's `PROFILE_PNPM_WORKSPACE` template), and a symlink-free npm production tree for `dshmarket@1.14.1`, `dsh-find-plugin@0.3.7`, and `dsh-anchored-subagent@0.3.0`. `dsh-anchored-subagent` is GitHub-only: the seed pins a codeload tarball URL at an exact commit so npm resolves it at packaging time and the target machine never needs git (the `github:` form would make pnpm re-resolve through git on the heal path). Plugin peer dependencies are deliberately not vendored — they resolve at runtime from the packaged DSH closure healed into `$DSH_HOME/profiles/node_modules`.
