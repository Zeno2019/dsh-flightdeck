...
- `scripts/prepare-profile-web.mjs` stages `build/profile-web/payload` at packaging time: the profile manifest (template bundles `@deepseek-ai/dsh-base` and `@deepseek-ai/dsh-web-app` plus the two approved plugins), an empty `cordis.patch.yml`, and a symlink-free npm production tree for `dshmarket@1.14.1` and `dsh-find-plugin@0.3.6`. Plugin peer dependencies are deliberately not vendored — they resolve at runtime from the packaged DSH closure healed into `$DSH_HOME/profiles/node_modules`.
