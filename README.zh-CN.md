...
安装包内置四个经审核的 DSH 插件，作为首次启动的 web profile 种子：
...
- `dsh-better-sidebar` 0.13.1 —— VSCode 式右侧栏（文件树、编辑器、终端、git 面板）直接织入 web UI；其 `node-pty` 为全平台 N-API prebuild，任何环节都不做原生编译

`scripts/prepare-profile-web.mjs` 在打包时把它们暂存为一棵无符号链接的 npm 生产依赖树，连同 profile 清单（模板包加上这四个插件）与一份空的 `cordis.patch.yml`。插件的 peer 依赖刻意不随包分发:运行时从打包的 DSH 闭包解析。首次启动时,仅当 `<userData>/harness/profiles/web` 尚不存在才会复制 profile;之后的启动绝不覆盖用户的改动。插件版本冻结在 prepare 脚本里--要升级就在那里改并重新构建。日后增删插件需要本机装有 pnpm(上游 `dsh plugin` 路径)。
